...
class PaisesRepository extends IRepository {
    ...
    async obtenerTodos() {
        return await paisModel.find({  creador: 'Alejandro Luna', 'name.nativeName.spa.official':{ $exists: true } }).lean() //trea unicamente los documentos creados por mi, y que tengan un nombre de pais
    }


    async borrarTodos() {
        return await paisModel.deleteMany({  creador: 'Alejandro Luna', 'name.nativeName.spa.official':{ $exists: true } }).lean() //borra unicamente los documentos dreados por mi, y que tengan un nombre de pais
    }
